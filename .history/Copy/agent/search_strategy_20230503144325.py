...
def mcts(node, max_simulations):
    while (max_simulations):
        ...
        # simulation
        while not_terminal(node): # need to write function for this  
            simulation(node)
...
def expand(node):
    actions = node.board.get_legal_actions(node.board, PlayerColor) # how do we get the current player colour?
